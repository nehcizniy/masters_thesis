...
filename_array[file_index]=filename;
        ...
        while(inputfile.getline(line, sizeof(line)))
        {
            ...
            else
            {
                ...
                yield_steeper[CurrentCentIndex][CurrentPtIndex] = yield[CurrentCentIndex][CurrentPtIndex] + 0.09 * yield[CurrentCentIndex][CurrentPtIndex] * (2*pt[CurrentCentIndex][CurrentPtIndex]/(int_low-int_high) + (int_high + int_low)/(int_high - int_low));
                yield_flat[CurrentCentIndex][CurrentPtIndex] = yield[CurrentCentIndex][CurrentPtIndex] + 0.09 * yield[CurrentCentIndex][CurrentPtIndex] * (2*pt[CurrentCentIndex][CurrentPtIndex]/(int_high-int_low) + (int_high + int_low)/(int_low - int_high));
...
leg->AddEntry(p2[11][0], "p+#bar{p}", "p");
leg->AddEntry((TObject*)0, " ", "");

leg->AddEntry(new TNamed(), "3He+Au","");
leg->AddEntry(p2[10][0], "p+#bar{p}", "p");
leg->AddEntry((TObject*)0, " ", "");
