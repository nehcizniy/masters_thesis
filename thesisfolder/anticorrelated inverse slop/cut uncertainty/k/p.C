...
filename_array[file_index]=filename;
        ...
        while(inputfile.getline(line, sizeof(line)))
        {
            ...
            else
            {
                ...
                yield_steeper[CurrentCentIndex][CurrentPtIndex] = yield[CurrentCentIndex][CurrentPtIndex] + 0.13 * yield[CurrentCentIndex][CurrentPtIndex] * (2*pt[CurrentCentIndex][CurrentPtIndex]/(int_low-int_high) + (int_high + int_low)/(int_high - int_low));
                yield_flat[CurrentCentIndex][CurrentPtIndex] = yield[CurrentCentIndex][CurrentPtIndex] + 0.13 * yield[CurrentCentIndex][CurrentPtIndex] * (2*pt[CurrentCentIndex][CurrentPtIndex]/(int_high-int_low) + (int_high + int_low)/(int_low - int_high));
    ...
    //cout << k << "\t" << integrated_yield[i] << "\t" << integrated_yield_corrected[i]<< "\t"<<  stat_integrated_yield[i]<< "\t"<<  sys_integrated_yield[i]<<endl;
        ...
        Double_t params_point[] = {0.3284,0.3};
        Double_t params_flat[] = {0.3284,0.3};
        Double_t params_steeper[] = {0.3284,0.3};
