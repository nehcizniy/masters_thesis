...
def read_data_from_excel(filename, min_pt=0.5, max_pt=2.0):
...
mlp_df = pd.DataFrame({
    'pt_bin': x_coor.flatten(),
    'y_MLP': np.array(y_coor).flatten(),
    'error_MLP': np.array(e_coor).flatten(),
})

mlp_df['pt_bin_rounded'] = mlp_df['pt_bin'].apply(lambda x: round(x, 2))  # Round to the nearest decimal as per your requirement.

output_summarized = mlp_df.groupby('pt_bin_rounded').agg(
    yield_avg=('y_MLP', 'mean'),
    error=('error_MLP', 'mean')
)

original_df = pd.DataFrame({
    'pt_bin': x_file1,  # Assuming x_file1 has center 'pt'
    'dpT': dx_1  # Assuming dx_1 has 'dpt' values
})
original_df['pt_bin_rounded'] = original_df['pt_bin'].apply(lambda x: round(x, 2))  # Match the rounded numbers for merging
...
final_df = final_df.drop(columns=['pt_bin_rounded'])  # Drop this auxiliary column.
