...
filename_array[file_index]=filename;
    ...
    {   
        ...
        // }
        // }
    //     TF1* fTsallis = new TF1("fTsallis","[0]*([2]-1.)*([2]-2.)*pow(([2]*[1]+sqrt(x*x+0.13970*0.13970))/([2]*[1]+0.13970),-[2])/(2.*3.1415*([2]*[1]+0.13970*([2]-1.))*([2]*[1]+0.13970))", xbin_fill[0], xbin_fill[M-1]);
...
TF1* fTsallis = new TF1("fTsallis","[0]*([2]-1.)*([2]-2.)*pow(([2]*[1]+sqrt(x*x+0.139570*0.139570))/([2]*[1]+0.139570),-[2])/(2.*3.1415*([2]*[1]+0.139570*([2]-1.))*([2]*[1]+0.139570))", 0., 20.);
    fTsallis->SetParameters(43.5, 0.112, 9.57);
    fTsallis->FixParameter(2, 9.57);
         for(int j=0;j<10;++j)
         {
             mg->Fit(fTsallis,"R");
         }
        
TF1* fTsallis_integral_function = new TF1("fTsallis_integral_function","[0]*([2]-1.)*([2]-2.)*pow(([2]*[1]+sqrt(x*x+0.139570*0.139570))/([2]*[1]+0.139570),-[2])/(2.*3.1415*([2]*[1]+0.139570*([2]-1.))*([2]*[1]+0.139570))", 0., 20.);
TCanvas* c1 = new TCanvas("c1","Pt vs Yield",800,600);
    c1->SetLogy();
    c1->SetLogx(0);
    gStyle->SetOptStat(0);
