...
cin >> int_high;
    ...
    mg->GetYaxis()->SetTitle("int.Yield");
...
filename_array[file_index]=filename;
    ...
    */}
    ...
    {   //stat_integrated_yield[i] = sqrt(stat_integrated_yield[i]);
        ...
        if(filename.Contains("phenix")){g[k] = new TGraphMultiErrors(NumCentIndex,Nch,integrated_yield_corrected,0,0,stat_integrated_yield, stat_integrated_yield);}
        if(filename.Contains("star")){g[k] = new TGraphMultiErrors(NumCentIndex,Nch,integrated_yield,0,0,stat_integrated_yield, stat_integrated_yield);}
